...
static string pstring(const char *s)
{
  if ((s[0] && s[1]) || isprint(s[0]))
    ...
    char buf[10];
    snprintf(buf, sizeof(buf), "\\%u", (unsigned)(unsigned char)s[0]);
    return string(buf);
  }
}
...
-t  Delete function tracepoints.\n";
	...
	if ((bflag && !interp.breakpoints.empty()) ||
	  ...
	  snprintf(msg, sizeof(msg), "This will clear all %spoints. Continue (y/n)?", ty);
	  if (yes_or_no(msg)) {
	  ...
	  snprintf(msg, sizeof(msg), "del: no %spoints\n", ty);
	  cerr << msg;
	}
      } else {
	for (list<string>::iterator it = args.l.begin();
	  ...
	  if (f > 0) {
		...
		snprintf(msg, sizeof(msg), "del: unknown %spoint '", ty);
		cerr << msg << s << "'\n";
	      }
	    } else {
	      if (bflag &&
